...
class RxBasebandSystem:
    def __init__(self, multi_ant_sys, Caz, param_est, case):
        ...
        self.synch_ref = Caz.ZChu0
        self.used_bins_synch = multi_ant_sys.used_bins_synch  # Same as Caz.used_bins.astype(int)
        ...
        if self.num_ant_txrx == 1:
            ...
            self.est_data_freq = np.zeros((1, lmax_d, len(self.used_bins_data)), dtype=complex)
        elif self.num_ant_txrx == 2 and self.MIMO_method == 'SPMult':
            self.est_data_freq = np.zeros((2, lmax_d, len(self.used_bins_data)), dtype=complex)
